...
def plot_kernel_spectrum(K: np.ndarray, outdir: str, title: str = "Kernel eigenvalue spectrum") -> str:
    ...
    plt.title(title)
    savedir = _timestamp_dir(outdir)
    path = os.path.join(savedir, "kernel_spectrum.png")
    plt.tight_layout(); plt.savefig(path, dpi=160); plt.close(fig)
    return path

def plot_lambda_curve(lams: Sequence[float], vals: Sequence[float], task: str, outdir: str) -> str:
...
def plot_confmat(y_true: np.ndarray, y_pred: np.ndarray, outdir: str) -> str:
